...
export default function Dashboard() {
  ...
  const discount = promoStatus === "applied" ? Math.round(subtotal * 0.2) : 0;
  const wheelDiscount = appliedWheelReward?.discount
    ? Math.round(subtotal * (appliedWheelReward.discount / 100))
    : 0;
  const deliveryFee = appliedWheelReward?.freeDelivery ? 0 : 400;
  const orderTotal = Math.max(0, subtotal - discount - wheelDiscount) + deliveryFee;
  ...
  const applyPromo = () => {
    if (promoCode.trim().toUpperCase() === "INDOMIECAFE") {
          ...
          {checkoutStep === 1 ? (
            <>
              ...
              <section className="promo-card" aria-labelledby="promo-heading">
                ...
                {promoStatus === "applied" && (
                  <p className="promo-feedback success">20% discount applied.</p>
                )}
                {promoStatus === "error" && (
